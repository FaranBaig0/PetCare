...
exports.bookAppointment = async (req, res) => {
    try {
        ...
        // Use uploaded file if present, otherwise null
    ...
    try {
        ...
        const prescription = await Prescription.findOne({
            ...
            include: [{
                ...
                where: { [Sequelize.Op.or]: [{ client_id: user_id }, { doctor_id: user_id }] }
            }]
        });

        if (!prescription) return res.status(404).json({ message: 'Prescription not found or access denied.' });
